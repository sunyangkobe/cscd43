...
/********** BEGIN NEW CODE **************/
/* Permenant usage count list */
static BufferUsage BufferUsageList[1000];
/********** END NEW CODE ***************/
			...
			bool *foundPtr)
{
	...
	*foundPtr = FALSE;
	...
	/***************************** BEGIN NEW CODE **********************************/
	/* Before delete the buffer from the table, keep the usagecount in a global list */
	isFound = FALSE;
	for (i = 0; i < sizeof(BufferUsageList)/sizeof(BufferUsage); i ++)
	{
		if (BufferUsageList[i].blockNum == buf->tag.blockNum)
		{
			isFound = TRUE;
			break;
		}
	...
	/* If the block can be found in the global list, update the value */
	if (isFound)
	{
		BufferUsageList[i].usagecount = buf->usagecount;
	}
	/* Initialize the element if it doesn't exist in the list */
	else
	{
		BufferUsage newUsage;
		newUsage.blockNum = buf->tag.blockNum;
		newUsage.usagecount = buf->usagecount;
		if (sizeof(BufferUsageList)/sizeof(BufferUsage) < 1000)
		{
			int j = sizeof(BufferUsageList)/sizeof(BufferUsage);
			BufferUsageList[sizeof(BufferUsageList)/sizeof(BufferUsage)] = newUsage;
		}
	}
	/**************************** END NEW CODE **************************************/
	...
	/**************************** BEGIN NEW CODE ************************************/
	/* Look up the target blockNum in the list, if found, update it.
	 * Otherwise, remain it to be the initial value 1. */
	buf->usagecount = 1;
	for (i = 0; i < sizeof(BufferUsageList)/sizeof(BufferUsage); i ++)
	{
		if (BufferUsageList[i].blockNum == buf->tag.blockNum)
		{
			buf->usagecount = BufferUsageList[i].usagecount + 1;
			break;
		}
	}
	/**************************** END NEW CODE **************************************/
