 ...
 * Currently, they are added on an LFU basis.
 */
static void
...
//      bf->freeNext = Free_List_Descriptor;
	...
	/****************** BEGIN NEW CODE ********************/
	/* look for the position the buffer should be inserted */
	curBf = &(BufferDescriptors[SharedFreeList->freePrev]);
	while ( curBf->buf_id != Free_List_Descriptor
			&& curBf->usagecount > bf->usagecount )
	{
		curBf = &(BufferDescriptors[curBf->freePrev]);
	}

	/* Now we can insert the bf after curBf in the freelist */

	/* change bf so it points to inFrontOfNew and its successor */
	bf->freePrev = curBf->buf_id;
	bf->freeNext = curBf->freeNext;
	/************* END NEW CODE **************/
